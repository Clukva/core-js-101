...
function isLeapYear(date) {
  return !(date.getUTCFullYear() % 4 !== 0
...
function timeSpanToString(startDate, endDate) {
  ...
  const mSec = `${date.getMilliseconds()}`.padStart(3, 0);
...
function angleBetweenClockHands(date) {
  ...
  const angleHours = 0.5 * (60 * (hour % 12) + min);
  const angelMin = 6 * min;
  let angel = Math.abs(angleHours - angelMin);
